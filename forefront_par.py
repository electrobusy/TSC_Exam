# -*- coding: utf-8 -*-
"""
Created on Wed Apr 14 14:01:49 2021
...
def function_par(X, t_train, s_train, u_train, param, optimizer='Nelder-Mead', bounds=Bounds(-5, 5, True)):
...
if __name__ == "__main__":
    ...
    args = [(X, t_train, s_train, u_train, param, 'Nelder-Mead', Bounds(-5, 5, True)) for (t_train,s_train,u_train) in zip(t_train_list,s_train_list,u_train_list)]
    ...
    start_time = time.time()
    p = Pool(mp.cpu_count())
    res = p.starmap(function_par, args)
    p.close()
    p.join()
    print("Pool of {} workers: ".format(mp.cpu_count()) + "Simulation ended at {}".format(time.time() - start_time) + " seconds")
    ...
    C_mean = np.mean(C_arr,axis=0).tolist()
    C_std = np.std(C_arr,axis=0).tolist()
    
    for (mean,std,label) in zip(C_mean,C_std,C_label):
        print('%s = %f +/- %f'%(label,mean,C_std[-1]*1.96))
